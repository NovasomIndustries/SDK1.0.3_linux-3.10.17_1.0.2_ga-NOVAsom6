...
/* Giovanni */
static const int rgb2rgb_coeff[5][3] = {
	{0x01, 0x01, 0x01},
	{0x01, 0x01, 0x01},
	{0x01, 0x01, 0x01},
	{0x01, 0x01, 0x01},	/* B0, B1, B2 */
	{0x01, 0x01, 0x01},	/* S0, S1, S2 */
};
...
/* modifiche temp GIOVANNI */
//	printk(" __ipu_dp_csc_setup :\n");
		...
		uint32_t out_pixel_fmt)
{
	...
	/* modifiche temp GIOVANNI */
//	printk("**** _ipu_dp_init called with ");
...
void _ipu_dp_set_csc_coefficients(struct ipu_soc *ipu, ipu_channel_t channel, int32_t param[][3])
{
	...
	dp_csc_param.mode = DP_COM_CONF_CSC_DEF_BOTH;	// GIOVANNI set enable BG and FG
				...
				bool enable, uint32_t color_key)
{
	...
	if (((ipu->fg_csc_type == RGB2YUV) && (ipu->bg_csc_type == YUV2YUV)) ||
		...
		printk("%s : color key 0x%x need change to yuv fmt\n",__FUNCTION__, color_key);
		...
		printk("%s : color key 0x%x changed to yuv fmt\n",__FUNCTION__, color_key);
	}

	if (enable) {
		printk("%s : color key 0x%x enabled\n",__FUNCTION__,color_key);
		...
		printk("%s : color key 0x%x disabled\n",__FUNCTION__,color_key);
		reg = ipu_dp_read(ipu, DP_COM_CONF(flow));
		ipu_dp_write(ipu, reg & ~DP_COM_CONF_GWCKE, DP_COM_CONF(flow));
	}
