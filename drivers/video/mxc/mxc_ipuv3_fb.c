...
/*
	printk("%s : cmd 0x%08X\n",__FUNCTION__,cmd);
	printk("%s : (void *)arg 0x%08X\n",__FUNCTION__,(void *)arg);
	printk("%s : sizeof((void *)arg) %d\n",__FUNCTION__,sizeof((void *)arg));
*/
	switch (cmd) {
	...
	case MXCFB_SET_LOC_ALPHA:
		{
			...
			if (la.enable)
				dev_dbg(fbi->device,
					...
					fbi->fix.id);
				printk("%s : Enable DP local alpha for %s!\n",__FUNCTION__,fbi->fix.id);
			break;
		}
	...
	case MXCFB_SET_CLR_KEY:
		{
			...
			retval = ipu_disp_set_color_key(mxc_fbi->ipu, mxc_fbi->ipu_ch,
							key.enable,
							key.color_key);
			printk("%s : Set color key to 0x%08x , 0x%08X\n",__FUNCTION__,key.enable,key.color_key);
